...
primary_access <- access_workers %>%
  filter(cutoff == auto_primary_cutoff_minutes)

if (nrow(primary_access) == 0) {
  stop("No automobile records found for the primary cutoff.")
}

map_data <- st_sf(
  primary_access,
  geometry = cell_to_polygon(primary_access$h3_id),
  crs = 4326
) %>%
  pivot_longer(
...
access_map <- wrap_plots(map_plots, ncol = 2) +
  plot_annotation(
    ...
    subtitle = paste0(
      "Jobs reachable within ", auto_primary_cutoff_minutes,
